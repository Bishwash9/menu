...
export default function QuickMenuPopup({ orderData, target, onClose }: QuickMenuPopupProps) {
    ...
    const handleCartSubmitted = () => {
        // Close popup after items are successfully added to cart
        onClose();
    };
        ...
        <div className="flex flex-col h-[85vh] w-full bg-white rounded-2xl overflow-hidden shadow-2xl">
            ...
            <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                <h2 className="text-xl text-primary font-bold">
                    Menu for {orderData.orderType === 'table' ? 'Table' : 'Room'} {orderData.identifier}
                </h2>
                <button
                    onClick={onClose}
                    className="cursor-pointer p-2 text-slate-400 hover:text-slate-600 transition-colors rounded-full hover:bg-slate-100"
                >
                    <X size={20} />
                </button>
            </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-80 border-l border-slate-200">
                    <CartSidebar
                        ...
                        onCartSubmitted={handleCartSubmitted}
                    />
                </div>
            </div>
        </div>
